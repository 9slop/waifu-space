...
export function createKyotoMap(scene: Scene): BabylonMapData {
  ...
  function createMachiyaHouse(
    ...
    if (hasShoji) {
      addBox(`${prefix}_ShF`, Math.min(w * 0.6, 5), h * 0.3, 0.12,
        new Vector3(pos.x, pos.y + h * 0.25, pos.z + d / 2 + 0.08), windowGlowMat, false);
  ...
  function createStoneLantern(name: string, pos: Vector3) {
    ...
    pl.diffuse = new Color3(1.0, 0.84, 0.46);
    pl.specular = new Color3(0.25, 0.18, 0.1);
    pl.intensity = 1.35;
    pl.range = 14;
  ...
  function createHangingLantern(name: string, pos: Vector3) {
    ...
    pl.specular = new Color3(0.2, 0.15, 0.08);
    pl.intensity = 1.2;
    pl.range = 12;
    lanternLights.push(pl);
  }
