...
describe('Global Store & RPG State (store.ts)', () => {
  ...
  describe('Cloud Sync (Supabase progress load)', () => {
    ...
    it('restores the selected holiday countries and cultural toggle from cloud settings_data', async () => {
      setState('user', { id: 'u1', username: 'CloudSettings', token: 'ws_cloud' });
      setState('settings', 'countryHolidays', []);
      setState('settings', 'showCulturalHolidays', false);
      ...
      vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
        ...
        json: async () => ({
          ...
          progress: {
            coins: 200,
            settings_data: { theme: 'neon', showCulturalHolidays: true, countryHolidays: ['JP', 'GB', 'ZZ', 'de'] }
          },
          inventory: [],
          showcaseItems: []
        })
